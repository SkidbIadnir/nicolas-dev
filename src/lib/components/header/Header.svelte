<script lang="ts">
  ...
  onMount(() => { lang = getCookie("lang") || "en"; });
...
<div class="navbar bg-base-300">
  <div class="navbar-start">
    <div class="dropdown">
      ...
      >
        <li><a href="#projects" class="duration-200">{tranTexts["Project"][lang]}</a></li>
        <!-- <li><a href="/skid" class="">Nicolas's links</a></li> -->

      </ul>
    </div>
    <a href="/#intropage" class="btn btn-ghost text-xl">{tranTexts["firstName"][lang]} {tranTexts["lastName"][lang]}</a>
  </div>
  <div class="navbar-center hidden lg:flex">
    <ul class="menu menu-horizontal px-1">
      <li>
        <a class="duration-200 font-semibold" href="/#projects">{tranTexts["Project"][lang]}</a>
      </li>
      ...
      <li><a href="/{u_data?.username}" class="font-semibold">{tranTexts["myPage"][lang]}</a></li>
  ...
  <div class="navbar-end gap-4">
    ...
    {#if $user}
      <a href="/" on:click={logout} class="btn hover:text-error"
        >{tranTexts["signOut"][lang]}<i class="fas fa-right-from-bracket"></i></a
      >
    {:else}
      <a href="/login" class="btn">{tranTexts["signIn"][lang]}<i class="fas fa-right-to-bracket"></i></a>
    {/if}
  </div>
</div>
